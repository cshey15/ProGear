...
exports.list = function(req, res) { 
	Issue.find().sort('-created').populate('user', 'displayName').populate('pro', 'name').populate('gear', 'name').exec(function(err, issues) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.issueByID = function (req, res, next, id) {
	...
	Issue.findById(id).populate('user', 'displayName').populate('pro', 'name').populate('gear', 'name').exec(function(err, issue) {
